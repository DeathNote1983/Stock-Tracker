...
const App = {
    ...
    async init() {
        ...
        // Load watchlist from server first
    ...
    setupEventListeners() {
        ...
        document.querySelectorAll('#popular-stock-tags .tag-btn').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                await Watchlist.addStock(symbol);
        ...
        document.querySelectorAll('#popular-crypto-tags .tag-btn').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                await Watchlist.addCrypto(cryptoId);
                btn.classList.add('active');
    ...
    startAutoRefresh() {
        this.intervals.data = setInterval(() => {
            if (UI.currentPage === 'dashboard') this.refreshData();
        }, CONFIG.REFRESH.STOCK);

        this.intervals.crypto = setInterval(() => {
            if (UI.currentPage === 'crypto') UI.renderCryptoPage();
        }, CONFIG.REFRESH.CRYPTO * 3);
    },
...
(function waitAndInit() {
    if (typeof Auth !== 'undefined' && typeof UI !== 'undefined' && typeof API !== 'undefined' && typeof News !== 'undefined') {
        Auth.initUI();
    } else {
