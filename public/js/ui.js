...
const Watchlist = {
    ...
    // Get watchlist (from cache or server)
    get() {
    ...
    async load() {
        try {
            ...
            console.warn('Failed to load watchlist from server:', error);
        }
        return this.get();
    },
    ...
    async addCrypto(id) {
        try {
            const res = await fetch('/api/watchlist', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ type: 'crypto', symbol: id }),
            });
            if (res.ok) {
                this._cache = await res.json();
            } else {
    ...
    async remove(type, id) {
        if (type === 'stock') await this.removeStock(id);
        else if (type === 'crypto') await this.removeCrypto(id);
    },
